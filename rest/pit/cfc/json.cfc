...
Version: 1.75 March 25, 2007
--->

<cfcomponent displayname="JSON" output="No"
hint="Used to encode/decode JSON. The native CFM functionality creates a format 
that doesn't work nicely with ExtJs"
>
	...
	<cffunction name="encode" access="remote" returntype="string" output="No"
		...
		<cfelseif IsStruct(_data)>
			...
			<cfloop from="1" to="#ArrayLen(arKeys)#" index="i">
				...
				<cfset jsonString = ListAppend(jsonString, '"' & LCase(arKeys[i]) & '":' & tempVal, ",") />
			</cfloop>
	...
	<cffunction name="validate" access="remote" output="yes" returntype="boolean"
			hint="Validates a JSON document against a JSON schema">
